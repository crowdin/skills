...
Use StrReplace scoped to the exact line being changed. Target the `"ai_context":""` substring:
...
If `ai_context` already has a value, target its full current value precisely.
