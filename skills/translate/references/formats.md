...
- When a language's drafting fans out in chunks (see "Large runs"), each worker reads its slice of the gap list the main thread wrote once (`gaps-<language id>.jsonl`, one record per string in file order), appends to its own part, `drafts-<language id>-<n>.jsonl`, and the main thread concatenates the parts into the language's ledger before step 5 and deletes them along with the gap list.
