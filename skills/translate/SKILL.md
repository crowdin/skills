...
- **Languages.** The ones the user named, otherwise every project language. Run `crowdin language list --verbose -o json` once and keep two fields per language: `code`, which under the default `--code id` is the Crowdin language id that `-l` takes, and `pluralCategoryNames`, which feeds the plural check in step 5. There is no separate locale field, and none is needed: the file each language lives in is whatever `crowdin config translations` printed above, with any `languages_mapping` from `crowdin.yml` already applied to the path. A mapping changes path placeholders, never the id `-l` takes, so the locale in a path is never derived from the id by hand and the id is never read off a path.
...
Before the upload, a library that can prove its source catalog is current gets asked, about the source catalog only: `lingui check sync --locale <source locale>` for Lingui 6.8 or later, and a failure means `lingui extract` first, so the strings that reach Crowdin are the ones in the code. The check compares each catalog file byte for byte with what extract would write, so it is never run over the target locales here: a target catalog Crowdin exported is formatted differently from Lingui's own output and fails for reasons that have nothing to do with the code. [references/formats.md](references/formats.md#library-checks) has the details and the fallback for an older Lingui. Upload first, so every string about to be translated exists in Crowdin — translations uploaded for a string Crowdin has never seen are ignored. Download second, so the local files reflect what reviewers have already decided. `--skip-untranslated-strings` makes untranslated entries come back absent instead of filled with source text; without it a key-value gap becomes invisible. When the project syncs a Crowdin branch, both commands take the same `-b`; whether it does is something the user or the CI workflow says, never a guess.

**Gate:** the target files are clean in git before the download, because the download overwrites them. Dirty target files are a stop-and-ask, never a silent stash. For a multilingual file (`.xcstrings`, a CSV with a `scheme`) the target is the source file itself, so the gate applies to it. `lingui extract` rewrites every locale's catalog, not only the source one, so the target catalogs it touched are dirty by this gate's definition: revert them (`git checkout -- <target files>`), since the download is about to replace them anyway, and leave the source catalog in the working tree for the user to commit with their strings.
...
Before the first draft in a language, settle what those sources leave open, once, and apply it everywhere: the register (`du` or `Sie`), one rendering for each recurring term the glossary does not cover, and number, date and unit formatting. A run with a placeholder style guide, an empty TM and a glossary without a column for the language still has to render "workspace" somehow; choosing one rendering and holding to it is not inventing a glossary translation (that rule forbids recording a term, not translating a word consistently). The choices go into the report as decisions for the reviewers, and into the conventions sheet when the run fans out (see "Large runs").
...
If the project's own scripts run a catalog compile (a `compile` script in `package.json`, a command the rules file `i18n-setup` rendered names), run it after the write so the running app shows the drafts. A project whose bundler plugin compiles the catalogs at build time has no such step, and running the compiler by hand only leaves stray output files; there the compile happens once, as the validator in step 5. Which command a library uses belongs to the library's own skill; this skill does not restate it.
...
2. **Format validity** of every touched target file, through whatever parser the project already has: the library's own compile when it has one (`lingui compile` for Lingui, whether or not the project runs it as a build step; delete its output files afterwards when the project does not), otherwise a format tool at hand (`jq` for JSON, `msgfmt -c` for PO, `xmllint` for XML, `plutil -lint` for Apple property lists, `yq` for YAML). A validator that dies on a large file (`msgfmt -c` killed in a sandbox) counts as unavailable: fall through to the next. When none is available, read the file back and say so in the report; a missing validator is never a pass. Where the library also counts gaps (`lingui check missing --locale <locale> --mode catalog` for Lingui), its exit `0` after the write is the proof that none was left behind.
...
Languages are independent, and so are chunks of one language. In an environment with a subagent tool, step 4 may fan out after step 3: one worker per target language, and for a language with more than about 150 gaps, one worker per chunk of 100 to 150 strings, because one worker drafting a thousand strings is exactly the single pass the warning above describes. Three rules keep the chunks consistent. A **conventions sheet** comes first: the main thread settles the register, the rendering of every recurring uncovered term and the formatting (step 4's decisions) before any worker starts, and every worker gets it. **One writer per target file**: a chunk worker writes only its own ledger part, `drafts-<language id>-<n>.jsonl`, and returns its table plus the terms it had to coin. **Reconcile before the merge**: the main thread lines up the coined terms across chunks, fixes the ones rendered two ways and adds them to the sheet, concatenates the parts into the language's ledger, writes the drafts into the target file once, and runs steps 5 and 6 once per language. Without subagent support the same work runs serially with an identical end state. Never required.
