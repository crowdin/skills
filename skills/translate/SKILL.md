...
- **Languages.** The ones the user named, otherwise every project language. Run `crowdin language list --verbose -o json` once and keep two fields per language: `code`, which is the language id that `-l` takes (the command's `--code` option defaults to `id`, with the project's language mapping already applied), and `pluralCategoryNames`, which feeds the plural check in step 5. The file each language lives in is whatever `crowdin config translations` printed above; the locale in that path is never derived from the id by hand.
