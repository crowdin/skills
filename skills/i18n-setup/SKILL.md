...
| 6 | Context | `ai_context` written for the new strings, a starting glossary uploaded, screenshots when opted in | token |
...
Two add-ons have to be *asked* rather than left off. The first is the **unwrapped-string lint rule**: phase 4's recall delegate installs it either way as a permanent dev dependency — its own skill recommends the install and treats the no-install path as the lesser one, so a plan that never raises it forces this orchestrator to answer a dependency question on the user's behalf, silently. Put it to them at Decide — the rule catches the next unwrapped string, the audit alone catches only today's — and record either answer, so phase 4 hands the delegate a decision instead of inheriting one.

The second is **screenshots**: phase 6 can delegate to the `generate-screenshots` skill, which captures the running app in a browser and uploads the images to Crowdin tagged to the strings they show — the strongest context a translator can get, and the one step in the journey that needs the user's hands mid-run: the app started, themselves logged in. Silence is not consent for that, so ask; the plan carries `generate_screenshots` only on a yes.
...
| 6 Context | `context_download`, `fill_ai_context`, `context_upload`, `context_status`, `generate_glossary`, `upload_glossary`, and `generate_screenshots` when the screenshots add-on was chosen |
...
Screenshots close the phase when the add-on was chosen at Decide: `generate_screenshots` delegates to the **`generate-screenshots`** skill, which plans the screens with the user, captures the running app, and uploads images auto-tagged to the strings they show — `ai_context` tells translators what a string means, a screenshot shows where it lives. It runs last because it is the only step here that needs the app running and the user present at the browser. `references/screenshots.md` is the handoff: what to pass, what stays the delegate's own, and what each outcome records.
